...
NTL.phys.data.year99 <- filter(NTL.phys.data, year4 %in% c(1999))
# what code do you need to use, based on the class of the variable?
class(NTL.phys.data$year4)
#year4 is an integer, so I need to use . . .

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday.year90s <- filter(NTL.phys.data, lakename %in% "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: We do not want the information in every column in a given row, so we do not filter using row numbers.
...
NTL.phys.data.July90s <- 
  NTL.phys.data %>%
  filter(lakename %in% "Tuesday Lake", year4 %in% c(1990:1999), daynum %in% c(182:212))
#could have also used month(sampledate) == 7)
...
NTL.phys.data.surface.temp <- 
  NTL.phys.data.July90s %>%
  filter(depth %in% 0.00) %>%
  filter(!is.na(temperature_C)) %>%
  summarize(mean_temp <- mean(temperature_C))
#could also use drop_na(temperature_C)
...
NTL.phys.data.irradiance.gathered <- 
  NTL.phys.data %>%
  pivot_longer(irradianceWater:irradianceDeck,
               names_to = "irradiance",
               values_to = "raditation")
