...
vector1[3] #this is to ge the third item out of vector 1. So it returns 5.3 in the console.

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #num of rows and columns has to equate to num of items in your matrix 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #this is saying enter these by row. So it counts up by row.
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) #this gives the column and row names

matrix1[4, ] #this gives you the fourth row, with every column in that row
matrix1[ , 3] #all rows of data in col 3
matrix1[c(12, 14)] #12th and the 14th value if they were all listed out. 
matrix1[c(12:14)] #colon includes value in between those two values, so 12th THROUGH 14th item
matrix1[2:4, 1:3] #things with the colon are called slices. 2nd - 4th row, and 1st - 3rd column.

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) #values for this matrix are taken from the cells
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] #each item in list needs a name. Semi colon allows us to put two commands on one line in r.

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector? - numeric
e <- c("red", "white", "red", NA) # What type of vector? -string
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? - boolean/binary
dataframe1 <- data.frame(d,e,f) #can put them tg bc they're all the same length
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #colnames also works 

dataframe1[1:2,] #slice, rows 1 and 2 and all columns 
...
> Answer: vectors appear under "values." Matrices, lists, and dataframes appear under "data."

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: It returns the vector (list of values) of the column I choose after the $.
...
```{r DataExercise-1a}
highs <- c(64, 71, 72, 64, 51, 52, 62, 64, 67, 74)
lows <- c(42, 49, 60, 46, 33, 32, 41, 47, 52, 54)
...
```{r DataExercise-1c}
df_Temps <- data_frame(highs, highsC, lows, lowsC)
#come back here and change column names using col names 
...
today <- Sys.Date() #date object, returns as UTC
format(today, format = "%B")
format(today, format = "%Y") #can keep going with this, just google r date formats
#format(today, format = "")
#format(today, format = "")
...
year(date_obj_past)) #%% gives you the remainder of whatever you're dividing, in this case div by 100 
