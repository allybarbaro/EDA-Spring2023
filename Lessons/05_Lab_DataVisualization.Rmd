...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    ) #paste brings together strings. Paste Zero does not insert spaces, while Paste does.
#expression command - MathJax. It's a little like markdown but different. It's a way of making mathematical formulas. This command says this is a mathematical expression, not just text.
...
ggsave(
  ...
  units = "in", 
  dpi = 300
  ) #it creates the folder if you don't already have one (so in this case it created the Output folder)
...
#if you're having trouble with your code going over on your knit doc, just hit enter after the comma and it will bump it down to the next line.
...
class(PeterPaul.chem.nutrients$month) #could also use glimpse command
...
#this part is helpful for the assignment!!!!!! Scale x discrete is telling the code you want to see all month factor values, even ones that don't have any data records.
...
the_plot <- PeterPaul.chem.nutrients %>%
 ...
 #finalized code for this part is in John's powerpoint
...
# 1. 
# Plot surface temperatures by day of  year. #temp on y, daynum on x axis
  daynum.temp.plot <- PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
  ggplot(aes(x = daynum, y = temperature_C, color = year4)) +
  geom_point(size=0.5, alpha=0.5) +
  facet_wrap(vars(lakename), ncol = 2) +
  labs(title = "Surface Temperature vs Day of Year by Year",
       x ="Day of Year", 
       y = "Surface Temperature C",
       color = "Year")

print(daynum.temp.plot)
#alpha sets the transparency of your dots/lines/etc
