...
#"sequencing using the date" in order to ensure no NAs, not just summary ????
...
ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) +
  ...
  labs(x = "Date", y = "Temperature in Celsius")
...
#could also have done start = c(1901,1) instead of doing this ^
Temperature_ts <- ts(BR_complete$Temperature_C, start = c(f_year, f_month), frequency = 12)
...
ggplot(Temperature_components) +
  ...
  labs( x = "Date", y = "Temperature in Celsisus")


ggplot(Rainfall_components) +
  geom_line(aes(x = Date, y = Observed), size = 0.25) +
  geom_line(aes(x = Date, y = trend), color = "red") +
  labs( x = "Date", y = "Rainfall in cm")
...
Temperature_trend <- trend::smk.test(Temperature_ts)
summary(Temperature_trend)
#P value is less than 0.05 (when we typed temperature_trend in the console) so we accept alternative hypothesis, which means positive is statistically significant (this means increasing trend)
