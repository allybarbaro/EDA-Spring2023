...
```{r visualization}
#Exercise 1:
ozone.dens.plot <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c('darkgreen', 'darkblue')) +
  labs(x = 'Ozone AQI Values', y = 'Density', fill = '')
  
print(ozone.dens.plot)

#2.
ozone.box.plot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("b", "a")) +
  labs(y = 'Ozone AQI Values')
  
print(ozone.box.plot)

#3
ozone.violin.plot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
...
#Exercise 2: Run a linear regression PM2.5 (y) by Ozone (x). Find the p-value and R-squared value. 

PM.ozone.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(PM.ozone.regression)

#P - value = 2.2e-16, R-squared value = 0.1314
...
#se = false gets rid of the gray halo of standard error that surrounds your line of best fit
...
EPAair.subset <- 
  ...
  drop_na()
#you cannot have NAs when trying to compute correlation

EPAairCorr <- cor(EPAair.subset)
corrplot.mixed(EPAairCorr, upper = "ellipse")
#EPAaircorr creates a matrix (table)
