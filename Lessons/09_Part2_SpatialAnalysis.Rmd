...
upper_neuse_sf <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapview(huc8_sf, col.regions = "orange") +
  mapview(upper_neuse_sf, col.regions = "purple", alpha = 1)

ggplot() +
  geom_sf(data = huc8_sf, fill = "purple", color = "gray") +
  geom_sf(data = upper_neuse_sf, fill = "orange", alpha = 1)
...
#colClasses for when we have a numeric column we dont want to treat as numbers (aka want it to be a factor or character)
...
final_the_hucs <- huc8_sf_utm %>%
  st_intersection(the_triangle)

mapview(final_the_hucs) + mapview(the_triangle, alpha.regions = 0, color = 'red')

ggplot()+
  geom_sf(data = the_triangle, fill = "purple") +
  geom_sf(data = final_the_hucs)
#this is not fully right aesthetically ^
