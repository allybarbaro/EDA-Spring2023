...
```{r project.setup, message=FALSE, echo=TRUE, results='hide'}
#Import libraries
...
```{r boxplot18, fig.height=2.5, fig.align='left', fig.cap= "Ozone Concentration by Month, 2018 ", echo=FALSE}

ozone.2018.data <- epa_data %>%
  filter(Year == 2018) %>%
  drop_na()

ozone.2018.plot <- ozone.2018.data %>%
  ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone, group = Month)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone (ppm)", title = "Ozone Concentrations per Month in 2018") +
  scale_x_discrete(drop=FALSE)
print(ozone.2018.plot)

```

```{r boxplot 19, fig.height=2.5, fig.align='left', fig.cap= " Ozone Concentration by Month, 2019", echo=FALSE}

ozone.2019.data <- epa_data %>%
  filter(Year == 2019) %>%
  drop_na()

ozone.2019.plot <- ozone.2019.data %>%
  ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone, group = Month)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone (ppm)", title = "Ozone Concentrations per Month in 2019") +
  scale_x_discrete(drop=FALSE)
print(ozone.2019.plot)
...
https://github.com/allybarbaro/EDA-Spring2023
